...
import UnderlinrInput from "./UnderlinrInput";
...
export default function CreateGroup({ setShowCreateGroup }) {
  ...
  return (
    <div className="createGroupAnimation flex0030 relative h-full z-40">
        ...
        <div className="mt-5">
            {/* Return / close button */}
            <button className="btn w-6 h-6"
                    onClick={() => setShowCreateGroup(false)}
            >
                <ReturnIcon className="fill-white"/>
            </button>
            {/* Group name input */}
            <UnderlinrInput name={name} setName={setName}/>
            {/* Multiple select */}
            <MultipleSelect 
            ...
            />
            {/*Create group button */}
            <div className="absolute bottom-1/2 left-1/2 -translate-x-1/2 max-h-fit">
                <button
